...
const CRMKanban = () => {
  ...
  const handleUpdateLead = async () => {
    ...
    setEditOpen(false); setEditLead(null);
    fetchData();
  };
  ...
  const handleAddActivity = async () => {
    ...
    await supabase.from("activities").insert({
      user_id: user.id, lead_id: editLead.id,
  ...
  const handleAddStage = async () => {
    ...
    await supabase.from("pipeline_stages").insert({ user_id: user.id, name: newStageName, position: maxPos, color: newStageColor });
  ...
  const handleUpdateStage = async () => {
    ...
    toast({ title: "Etapa atualizada!" }); setEditingStage(null);
    fetchData();
  };
  ...
  const handleMoveStage = async (stageId: string, direction: "up" | "down") => {
    ...
    if (idx === -1 || (direction === "up" && idx === 0) || (direction === "down" && idx === stages.length - 1)) return;
  ...
  const addTagToLead = () => {
    ...
    if (!editLead.tags.includes(newTag.trim())) {
      setEditLead({ ...editLead, tags: [...editLead.tags, newTag.trim()] });
    }
    setNewTag("");
  };
  ...
  const leadFormFields = (data: any, setData: (d: any) => void, isNew = false, showStageSelect = false) => (
    <div className="space-y-3 max-h-[60vh] overflow-y-auto pr-1">
      ...
      {isNew && (
        <div><Label className="text-xs">Tags (separadas por vírgula)</Label><Input value={data.tags || ""} onChange={e => setData({ ...data, tags: e.target.value })} placeholder="vip, urgente, novo" className="mt-1 bg-secondary/50 border-border" /></div>
      )}
            ...
            return (
              <div key={stage.id} className="flex-shrink-0 w-72 flex flex-col" onDragOver={e => e.preventDefault()} onDrop={() => handleDrop(stage.id)}>
                ...
                <div className="flex-1 space-y-2 min-h-[100px] rounded-lg p-2 bg-muted/30 border border-border/50">
                  {stageLeads.map(lead => (
                    <div key={lead.id} draggable onDragStart={() => setDraggedLead(lead.id)} onClick={() => { setEditLead({ ...lead }); setEditOpen(true); setEditTab("info"); }}
                      ...
                      <div className="flex items-center gap-1.5 mt-2 flex-wrap">
                        ...
                        {(lead.tags || []).slice(0, 2).map(tag => (
                          <span key={tag} className="text-[10px] bg-accent/20 text-accent-foreground px-1.5 py-0.5 rounded">{tag}</span>
                        ))}
                      </div>
                      {lead.value > 0 && (
