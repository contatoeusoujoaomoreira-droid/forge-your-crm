...
const SectionEditor = ({ section, onChange }: Props) => {
  ...
  const renderBackgroundEngine = () => (
    <div className="space-y-3 border-t border-border pt-4 mt-4">
      {renderSectionHeader("🎨", "Background Engine")}
      <div className="grid grid-cols-2 gap-3">
        <div>
          <Label className="text-[10px] text-muted-foreground">Cor de fundo</Label>
          <div className="flex gap-1.5 mt-1">
            <input type="color" value={c.bgColor || "#000000"} onChange={(e) => set("bgColor", e.target.value)} className="w-8 h-8 rounded-lg cursor-pointer border border-border bg-transparent shrink-0" />
            <Input value={c.bgColor || "#000000"} onChange={(e) => set("bgColor", e.target.value)} className="h-8 text-xs bg-secondary border-border flex-1" />
          </div>
        </div>
        <div>
          <Label className="text-[10px] text-muted-foreground">Cor do texto</Label>
          <div className="flex gap-1.5 mt-1">
            <input type="color" value={c.textColor || "#ffffff"} onChange={(e) => set("textColor", e.target.value)} className="w-8 h-8 rounded-lg cursor-pointer border border-border bg-transparent shrink-0" />
            <Input value={c.textColor || "#ffffff"} onChange={(e) => set("textColor", e.target.value)} className="h-8 text-xs bg-secondary border-border flex-1" />
          </div>
        </div>
      </div>
      <div className="grid grid-cols-2 gap-3">
        <div>
          <Label className="text-[10px] text-muted-foreground">Cor destaque</Label>
          <div className="flex gap-1.5 mt-1">
            <input type="color" value={c.accentColor || "#84CC16"} onChange={(e) => set("accentColor", e.target.value)} className="w-8 h-8 rounded-lg cursor-pointer border border-border bg-transparent shrink-0" />
            <Input value={c.accentColor || "#84CC16"} onChange={(e) => set("accentColor", e.target.value)} className="h-8 text-xs bg-secondary border-border flex-1" />
          </div>
        </div>
        <div>
          <Label className="text-[10px] text-muted-foreground">Padrão de fundo</Label>
          <select value={c.bgPattern || "none"} onChange={(e) => set("bgPattern", e.target.value)} className="w-full h-8 text-xs bg-secondary text-foreground border border-border rounded-lg px-2 mt-1">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <select value={c.animation || "none"} onChange={(e) => set("animation", e.target.value)} className="w-full h-8 text-xs bg-secondary text-foreground border border-border rounded-lg px-2 mt-1">
  ...
  const renderTextEditor = () => (
    <div className="space-y-3 border-t border-border pt-4 mt-4">
      {renderSectionHeader("✏️", "Tipografia")}
      <div className="grid grid-cols-2 gap-3">
        <div>
          <Label className="text-[10px] text-muted-foreground">Fonte</Label>
          <select value={c.fontFamily || "Inter"} onChange={(e) => set("fontFamily", e.target.value)} className="w-full h-8 text-xs bg-secondary text-foreground border border-border rounded-lg px-2 mt-1">
        ...
        <div>
          ...
          <select value={c.headingWeight || "700"} onChange={(e) => set("headingWeight", e.target.value)} className="w-full h-8 text-xs bg-secondary text-foreground border border-border rounded-lg px-2 mt-1">
  ...
  const renderActionManager = () => (
    <div className="space-y-3 border-t border-border pt-4 mt-4">
      {renderSectionHeader("🔗", "Action Manager")}
      <div className="grid grid-cols-2 gap-3">
        <div>
        ...
        <div>
          ...
          <select value={c.ctaAction || "link"} onChange={(e) => set("ctaAction", e.target.value)} className="w-full h-8 text-xs bg-secondary text-foreground border border-border rounded-lg px-2 mt-1">
      ...
      {c.ctaAction === "whatsapp" ? (
        <div>
          <Label className="text-[10px] text-muted-foreground">Número WhatsApp</Label>
        ...
        <div>
          <Label className="text-[10px] text-muted-foreground">ID do elemento</Label>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <Label className="text-[10px] text-muted-foreground">Estilo botão</Label>
          <select value={c.btnStyle || "solid"} onChange={(e) => set("btnStyle", e.target.value)} className="w-full h-8 text-xs bg-secondary text-foreground border border-border rounded-lg px-2 mt-1">
            {btnStyles.map(s => <option key={s.value} value={s.value}>{s.label}</option>)}
          </select>
        </div>
        <div>
          <Label className="text-[10px] text-muted-foreground">Tamanho botão</Label>
          <select value={c.btnSize || "md"} onChange={(e) => set("btnSize", e.target.value)} className="w-full h-8 text-xs bg-secondary text-foreground border border-border rounded-lg px-2 mt-1">
    ...
    <div className="p-4 space-y-4">
      ...
      {(section.section_type === "hero" || section.section_type === "cta") && (
        <div className="space-y-3">
          ...
          {section.section_type === "hero" && (<div><Label className="text-xs text-muted-foreground">Subtítulo</Label><Textarea value={c.subtitle || ""} onChange={(e) => set("subtitle", e.target.value)} className="text-xs bg-secondary border-border mt-1" rows={2} /></div>)}
          {section.section_type === "cta" && (<div><Label className="text-xs text-muted-foreground">Descrição</Label><Textarea value={c.description || ""} onChange={(e) => set("description", e.target.value)} className="text-xs bg-secondary border-border mt-1" rows={2} /></div>)}
          {section.section_type === "hero" && (<div><Label className="text-xs text-muted-foreground">Badge</Label><Input value={c.badge || ""} onChange={(e) => set("badge", e.target.value)} className="h-8 text-xs bg-secondary border-border mt-1" /></div>)}
          {renderActionManager()}
          {renderTextEditor()}
        </div>
      )}
        ...
        <div className="space-y-3">
          ...
          {(c.plans || []).map((plan: any, idx: number) => (
            <div key={idx} className="bg-secondary/50 rounded-lg p-3 space-y-2 relative">
              ...
              <label className="flex items-center gap-2 text-xs text-foreground"><input type="checkbox" checked={plan.highlight || false} onChange={(e) => { const p = [...(c.plans || [])]; p[idx] = { ...p[idx], highlight: e.target.checked }; set("plans", p); }} className="accent-primary" /> Destacar</label>
            </div>
          ))}
          <Button variant="ghost" size="sm" className="w-full h-7 text-xs text-primary hover:bg-primary/10" onClick={() => set("plans", [...(c.plans || []), { name: "", price: "", features: [], ctaText: "Escolher", ctaUrl: "#", highlight: false }])}><Plus className="w-3 h-3 mr-1" /> Plano</Button>
        </div>
      )}
